...
export default function HomeSearch() {
  ...
  const randomSearch = async (e) => {
    ...
    const response = await fetch('https://random-word-api.herokuapp.com/word')
      .then((res) => res.json())
      .then((data) => data[0]);//first item of the list
    if (!response) return;
    const encodedSearchTerm = encodeURIComponent(response);//usecase of JS METHOD encodeURIComponent
    router.push(`/search/web?searchTerm=${encodedSearchTerm}`);
